...
采用类似 GitHub / Twitter 的**用户语义化层级 URL 体系**：
...
| `/[userId]` | `src/routes/[userId]/+page.svelte` | **用户公开主页 (User Profile & Wall)** | 用户个人目标墙：展示用户头像、昵称、个人 Todo 统计（全部、进行中、已达成、总获赞数）以及该作者发布的所有公开 Todo 列表 |
| `/[userId]/[todoId]` | `src/routes/[userId]/[todoId]/+page.svelte` | **Todo 独立详情页 (Detail & Share)** | 单条 Todo 专属围观/分享页（支持社交媒体分享），展示完整规划内容、起止日期时间线、点赞人完整名单与 Emoji 互动区，作者在此页亦可直接流转状态与编辑 |
...
├── lib/
│   ├── types/
│   │   ├── todo.ts               # 前端 Todo、Reaction、Author 等 TypeScript 接口
│   │   └── user.ts               # 用户资料与本地会话类型
│   ├── constants/
...
│   ├── stores/
│   │   ├── user.svelte.ts        # 基于 Svelte 5 的当前用户身份持久化 Store (localStorage)
...
│       ├── todo/                 # Todo 业务组件
│       │   ├── CreateTodoCard.svelte # 极简发布卡片（单行收起/聚焦展开）
│       │   ├── TodoCard.svelte       # 单条 Todo 卡片（状态流转、作者操作、互动栏）
...
│       └── user/                 # 用户相关组件
│           ├── UserIdentityBar.svelte# 顶部导航右侧身份展示与切换
...
└── routes/
    ...
    ├── +page.svelte              # 待办广场主页
    ├── [userId]/
    │   ├── +page.svelte          # 用户公开主页 (/[userId])
    │   └── [todoId]/
    │       └── +page.svelte      # Todo 详情与围观页 (/[userId]/[todoId])
