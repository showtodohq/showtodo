<script lang="ts">
	...
	const isTrending = $derived(pathname.startsWith('/trending') || pathname.startsWith('/goals'));
		...
		<div class="flex items-center justify-center">
			...
			>
				...
				<a
					...
					title="Trending"
					aria-label="Trending"
					class="flex items-center gap-1.5 px-2.5 sm:px-3 py-1 rounded-lg transition-all {isTrending
						? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 shadow-2xs font-semibold'
						: 'text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100'}"
				>
					<svg class="h-3.5 w-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width={isTrending ? 2.2 : 1.8}>
