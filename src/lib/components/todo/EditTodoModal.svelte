<script lang="ts">
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		if (!email) {
			toast.error('未找到作者邮箱身份，无法修改');
			return;
		}

		if (!content.trim()) {
			toast.warning('Todo 内容不能为空');
			return;
		}
		...
		<form onsubmit={handleSubmit} class="space-y-4">
			...
			<div>
					...
					{#each TODO_STATUSES as st}
						...
						<button
							...
							st.id
								...
								: isAvailable
									? 'bg-zinc-50 dark:bg-zinc-800/50 border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100'
				...
				{#if allowedStatuses.length === 1}
					<p class="text-[11px] text-zinc-400 mt-1">当前处于终态，无法再切换状态</p>
				{/if}
			</div>
			...
			<div>
				...
				<div class="flex flex-wrap gap-1.5">
					<button
						...
						class="px-2 py-1 rounded-md text-xs border transition-colors {category === null
					...
					{#each CATEGORIES as cat}
						<button
							...
							class="px-2 py-1 rounded-md text-xs border transition-colors {category === cat.id
			...
			<div class="pt-3 flex justify-end gap-2 border-t border-zinc-100 dark:border-zinc-800">
				<button
					type="button"
					onclick={onClose}
					class="px-3.5 py-1.5 text-xs font-medium rounded-lg text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:text-white dark:hover:bg-zinc-800 transition-colors"
				>
					取消
				</button>
				<button
					...
					class="inline-flex items-center gap-1.5 px-4 py-1.5 text-xs font-medium rounded-lg bg-zinc-900 text-white hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100 transition-colors disabled:opacity-50"
				>
					{#if isSubmitting}
