<script lang="ts">
	...
	let searchTriggerRef = $state<HTMLElement | null>(null);
	let popoverPanelRef = $state<HTMLElement | null>(null);
	...
	function handleClickOutside(e: MouseEvent) {
		...
		// 只有点击在【浮窗面板内部】或【搜索触发按钮胶囊自身】才算组件内部
		// 点击工具栏那一行的空白区域、左侧 Tabs 或页面其余部分，一律自然关闭浮窗
		const isInsidePanel = popoverPanelRef?.contains(target);
		const isInsideTrigger = searchTriggerRef?.contains(target);
...
>
	...
	<div class="flex items-center justify-between gap-3 w-full">
		...
		<div class="flex items-center gap-2 shrink-0 ml-auto">
			<div bind:this={searchTriggerRef} class="inline-flex items-center">
				{#if isSummary}
